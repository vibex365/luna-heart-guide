...
export const GiftCollection = ({ partnerLinkId, partnerId }: GiftCollectionProps) => {
  const { receivedGifts, sentGifts, receivedLoading, sentLoading } = useGiftStore(
    partnerLinkId,
    partnerId
  );
        ...
        <div className="p-4 space-y-3">
          {allGifts.map((gift, index) => (
            <GiftHistoryItem
              key={gift.id}
              gift={gift}
              isSent={sentGifts.some((g) => g.id === gift.id)}
              index={index}
            />
          ))}
        </div>
...
const GiftHistoryItem = ({ gift, isSent, index }: GiftHistoryItemProps) => {
  return (
    <motion.div
